...
window.GRAMMAR_CHECK_DATA = {
  ...
  questions: [
    ...
    ["conjunction", "接続詞の基本的な働きとして正しいものは？", ["語句や節をつなぎ、前後の関係を示す", "名詞だけを修飾する", "動詞の時制を変える", "単語の数をそろえる"], "語句や節をつなぎ、前後の関係を示す", "接続詞は語句や節をつなぎ、原因・結果・条件・対比など、前後の内容の関係を示す。"],
    ...
    ["infinitive", "不定詞の否定形として正しいものは？", ["not to + 動詞の原形", "to not + 動詞の過去形", "not + 動詞の三単現", "no + to + 動詞の原形"], "not to + 動詞の原形", "不定詞を否定するときは、基本的にnotをtoの前に置いてnot to doとする。"],
    ...
    ["gerund", "動名詞が動詞の目的語になるとき、文中でどのような働きをしているか？", ["名詞相当の働き", "形容詞として名詞を修飾する働き", "副詞として動詞を修飾する働き", "接続詞として節をつなぐ働き"], "名詞相当の働き", "動名詞は動詞由来の形だが、目的語になると名詞相当の働きをする。I enjoy reading. ではreadingがenjoyの目的語になっている。"],
    ["gerund", "-ing形が名詞として文中で働くとき、何と呼ぶか？", ["動名詞", "現在分詞", "過去分詞", "不定詞"], "動名詞", "Reading books is useful. のReadingのように、-ing形が主語・目的語・補語など名詞相当の働きをするとき動名詞と呼ぶ。"],
    ...
    ["verb_form", "状態を表す動詞を進行形にするか判断するとき、何を確認するか？", ["動作が一時的に進行中か、状態として続いているか", "主語が必ず人か", "目的語が複数か", "文に前置詞があるか"], "動作が一時的に進行中か、状態として続いているか", "進行形は一時的な動作の途中を表す。所有・知識・感情などの状態を表す動詞は、通常は進行形にせず現在形で表す。"],
    ...
    ["conjunction", "条件を表す副詞節の働きとして正しいものは？", ["条件が成り立つ場合の主節の結果を示す", "名詞の数を表す", "主語の性質を説明する", "動詞を必ず過去形にする"], "条件が成り立つ場合の主節の結果を示す", "ifなどが導く条件の副詞節は、どの条件で主節の内容が成り立つかを示す。未来のことでも、時・条件の副詞節では現在形を使うことが多い。"],
