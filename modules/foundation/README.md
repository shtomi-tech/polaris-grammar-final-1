...
- 予習資料: `data/preparation-<preparationUnit>.md`
...
3. その `lessonId` の `preparationUnit` に対応する `data/preparation-*.md` を開き、追加した問題が「予習資料だけで解ける」状態か確認する。足りない論点は見出しを追加する（`##` 見出し1つにつき `:::check` を1つ）。
4. 下の検証を実行する。
